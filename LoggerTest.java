...
//TODO add instance variable(s)
    ...
    void printResults() {
        ...
        LogProcessor<T> firstLogProcessor = (c) -> {
            ArrayList<T> logs = new ArrayList<>();
            for(T l : c){
                if (l.getType().equals("INFO")) logs.add(l);
            }
            return logs;
        };
        ...
        LogProcessor<T> secondLogProcessor = (logsList) -> {
            ArrayList<T> logs = new ArrayList<>();
            for(T l : logsList){
                if (l.getMessage().length() < 100) logs.add(l);
            }
            return logs;
        };

        LogProcessor<T> thirdLogProcessor = (logsList) -> {
            for(int i = 0; i<logsList.size(); i++){
                for(int j = 0; j<logsList.size()-1; j++){
                    if(logsList.get(j).getTimestamp() > logsList.get(j+1).getTimestamp()){
                        T tmp = logsList.get(j);
                        logsList.set(j, logsList.get(j+1));
                        logsList.set(j+1, tmp);
                    }
                }
            }
            return logsList;
